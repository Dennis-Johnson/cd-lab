...
int ungetToken(FILE *fin, Token *token)
{
    ...
    if (token == NULL)
    {
        fprintf(stderr, "Error: NULL token pointer.\n");
        return 1;
    }

    printf("Unget token %s: %s\n", TokenTypeToString(token->type), token->token_name);

    int pushback_len = strlen(token->token_name);
    fseek(fin, -1 * pushback_len, SEEK_CUR);
